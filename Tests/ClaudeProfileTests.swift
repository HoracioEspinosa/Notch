...
final class ClaudeProfileTests: XCTestCase {
    ...
    /// places from one launch to the next. Every profile is signed in here:
    /// order is what this is about, and the real keychain holds nothing for a
    /// home under `/tmp`.
    func testDiscoveryFindsEveryUsedProfileInAStableOrder() throws {
        ...
        let found = ClaudeProfile.discover(home: home, hasCredential: { _ in true })
    ...
    func testAnyFirstRunMarkerCounts() throws {
        ...
        XCTAssertEqual(ClaudeProfile.discover(home: home, hasCredential: { _ in true }).map(\.id),
    ...
    /// A fresh Mac has no `~/.claude` and no token to go with it, and still has
    /// to get the ring: it is the only thing that tells the person Claude Code
    /// is waiting to be signed in to.
    func testTheDefaultSurvivesWhenItIsTheOnlyProfile() throws {
        let home = try home([:])
        XCTAssertEqual(ClaudeProfile.discover(home: home).map(\.id), ["claude"])
    }

    /// Someone who signed in only under `~/.claude-work` still has a `~/.claude`
    /// left behind by Claude Code itself. Drawing it put a grey ring in the
    /// notch that no amount of signing in would ever fill.
    func testTheDefaultIsDroppedWhenAnotherProfileHoldsTheOnlyToken() throws {
        let home = try home([
            ".claude": ["settings.json"],
            ".claude-work": ["settings.json"]
        ])
        let found = ClaudeProfile.discover(home: home, hasCredential: { $0.slug != nil })
        XCTAssertEqual(found.map(\.id), ["claude-work"])
    }

    /// Both accounts signed in, both drawn: the default is only dropped for
    /// want of a token, never for the company it keeps.
    func testTheDefaultStaysBesideAnotherProfileWhenItIsSignedIn() throws {
        let home = try home([
            ".claude": ["settings.json"],
            ".claude-work": ["settings.json"]
        ])
